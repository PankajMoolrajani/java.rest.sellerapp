...
@Path("/user/update")
public class UserCreateFormUpdate 
{
	@POST
	@Path("/form_data")
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.TEXT_PLAIN)
	public String updateUser(UserFormBean userBean)
	{	
		Map<String,String> map = new HashMap<String,String>();
		System.out.println(userBean.getUserId()+" "+userBean.getUserCatId()+" "+userBean.getFirstName());
		createUser(map,userBean);
		
		return "updation process";
	}
	
	public void createUser(Map<String,String> map, UserFormBean userBean)
	{
		...
		try
		{
			...
			psUser.setInt(1,userBean.getUserCatId());
			psUser.setString(2,userBean.getFirstName());
			psUser.setString(3,userBean.getLastName());
			psUser.setString(4,userBean.getEmailId());
			psUser.setString(5,userBean.getPhoneNumber());
			psUser.setInt(6,userBean.getUserId());			
			psUser.execute();
